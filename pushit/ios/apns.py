...
try:
    ...
    from socket import sslerror as SSLError  # noqa
...
class APNs(object):
    ...
    @staticmethod
    def unpacked_ushort_big_endian(bytes_value):
        """
    ...
    @staticmethod
    def unpacked_uint_big_endian(bytes_value):
        """
    ...
    @staticmethod
    def unpacked_char_big_endian(bytes_value):
        """
        Returns an unsigned char from a packed big-endian (network) byte array
        """
        return struct.unpack('c', bytes_value)[0]
...
class GatewayConnection(APNsConnection):
    ...
    def __init__(self, use_sandbox=False, **kwargs):
        ...
        self.port = 2195
        if self.enhanced is True:  # start error-response monitoring thread
            self._last_activity_time = time.time()
    ...
    def _get_notification(self, token_hex, payload):
        ...
        notification = (zero_byte + token_length_bin + token_bin + payload_length_bin + payload_json)
    ...
    def _get_enhanced_notification(self, token_hex, payload, identifier, expiry):
        ...
        """
        token = binascii.a2b_hex(token_hex)
        payload = payload.json()
        fmt = ENHANCED_NOTIFICATION_FORMAT % len(payload)
        notification = struct.pack(fmt, ENHANCED_NOTIFICATION_COMMAND, identifier, expiry,
    ...
    class ErrorResponseHandlerWorker(threading.Thread):
        ...
        def run(self):
            while True:
                    ...
                    if len(rlist) > 0:  # there's some data from APNs
                        with self._apns_connection._send_lock:
                            ...
                            if len(buff) == ERROR_RESPONSE_LENGTH:
                                command, status, identifier = struct.unpack(ERROR_RESPONSE_FORMAT, buff)
                ...
                except socket.error as e:  # APNS close connection arbitrarily
                    _logger.exception(
        ...
        def _resend_notifications_by_id(self, failed_identifier):
            fail_idx = Util.get_list_index_from_id(self._apns_connection._sent_notifications, failed_identifier)
